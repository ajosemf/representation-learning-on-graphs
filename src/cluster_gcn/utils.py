...
def preprocess_multicluster(adj,
  ...
  for _, st in enumerate(range(0, num_clusters, block_size)):
    ...
    for pt_idx in range(st + 1, min(st + block_size, num_clusters)):
      # by Augusto Jose, incluido o parser para int32
      # quando parts[pt_idx] eh um array vazio, o metodo np.concatenate altera o dtype para np.float64
      # na linha 145, ao acessar features[pt, :] ocorre um erro porque arrays de indices devem ser inteiros ou booleanos
      # pt = np.concatenate((pt, parts[pt_idx]), axis=0)
      pt = np.concatenate((pt, parts[pt_idx]), axis=0).astype(np.int32)
